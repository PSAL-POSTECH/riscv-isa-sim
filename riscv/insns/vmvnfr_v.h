...
for (reg_t vu_idx=0; vu_idx<n_vu; vu_idx++) {
  ...
  if (vd != vs2 && P.VU.vstart->read() < size) {
    ...
    if (off) {
      memcpy(&P.VU.elt<uint8_t>(vd + i, off, vu_idx, true),
            &P.VU.elt<uint8_t>(vs2 + i, off, vu_idx), P.VU.vlenb - off);
      i++;
    }

    for (; i < len; ++i) {
      memcpy(&P.VU.elt<uint8_t>(vd + i, 0, vu_idx, true),
            &P.VU.elt<uint8_t>(vs2 + i, 0, vu_idx), P.VU.vlenb);
    }
  }
