            ...
            for (h=0; h<block_shape[2]; h++) {
                for (w=0; w<block_shape[3]; w++) {
                    ...
                    if (element_size == 1) {
                        ...
                        static_cast<uint8_t*>(dma_tensor)[vlane_idx * vlane_stride * factor + n * p_dim_size[1] * p_dim_size[2] * p_dim_size[3] + c * p_dim_size[2] * p_dim_size[3] + h * p_dim_size[3] + w] = val;
                    } else if (element_size == 2) {
                        if (padding_flag || p_split_dim < vlane_split_axis)
                            continue;
                        uint16_t val = MMU.load_uint16(s_addr);
                        static_cast<uint16_t*>(dma_tensor)[vlane_idx * vlane_stride * factor + n * p_dim_size[1] * p_dim_size[2] * p_dim_size[3] + c * p_dim_size[2] * p_dim_size[3] + h * p_dim_size[3] + w] = val;
                    } else if (element_size == 4) {
                        if (padding_flag || p_split_dim < vlane_split_axis)
                            continue;
                        uint32_t val = MMU.load_uint32(s_addr);
                        static_cast<uint32_t*>(dma_tensor)[vlane_idx * vlane_stride * factor + n * p_dim_size[1] * p_dim_size[2] * p_dim_size[3] + c * p_dim_size[2] * p_dim_size[3] + h * p_dim_size[3] + w] = val;
                    } else if (element_size == 8) {
                        if (padding_flag || p_split_dim < vlane_split_axis)
                            continue;
                        uint64_t val = MMU.load_uint64(s_addr);
                        static_cast<uint64_t*>(dma_tensor)[vlane_idx * vlane_stride * factor + n * p_dim_size[1] * p_dim_size[2] * p_dim_size[3] + c * p_dim_size[2] * p_dim_size[3] + h * p_dim_size[3] + w] = val;
                    }
                }
            }
        }
    ...
    for (c=0; c<p_dim_size[1]; c++) {
        for (h=0; h<p_dim_size[2]; h++) {
            for (w=0; w<p_dim_size[3]; w++) {
                ...
                if (element_size == 1) {
                    uint8_t*_t val = static_cast<uint8_t*>(dma_tensor)[n * p_dim_size[1] * p_dim_size[2] * p_dim_size[3] + c * p_dim_size[2] * p_dim_size[3] + h * p_dim_size[3] + w];
                    MMU.store_uint8(d_addr, val);
                } else if (element_size == 2) {
                    uint16_t*_t val = static_cast<uint16_t*>(dma_tensor)[n * p_dim_size[1] * p_dim_size[2] * p_dim_size[3] + c * p_dim_size[2] * p_dim_size[3] + h * p_dim_size[3] + w];
                    MMU.store_uint16(d_addr, val);
                } else if (element_size == 4) {
                    uint32_t*_t val = static_cast<uint32_t*>(dma_tensor)[n * p_dim_size[1] * p_dim_size[2] * p_dim_size[3] + c * p_dim_size[2] * p_dim_size[3] + h * p_dim_size[3] + w];
                    MMU.store_uint32(d_addr, val);
                } else if (element_size == 8) {
                    uint64_t*_t val = static_cast<uint64_t*>(dma_tensor)[n * p_dim_size[1] * p_dim_size[2] * p_dim_size[3] + c * p_dim_size[2] * p_dim_size[3] + h * p_dim_size[3] + w];
